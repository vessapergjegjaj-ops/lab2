...
export default function Notifications(){
  ...
  const markRead = async (id) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Unable to mark as read')
    } finally {
      setMarkingId(null)
    }
  }
          ...
          <div key={notification.id || notification._id} className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ padding: '4px 10px', borderRadius: 999, background: notification.is_read ? '#334155' : '#0ea5a4', color: '#fff', fontSize: 12 }}>
