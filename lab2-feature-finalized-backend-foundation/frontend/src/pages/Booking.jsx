...
export default function Booking(){
  ...
  useEffect(()=>{
    ...
    matchService.getMatchById(matchId)
      ...
      .catch(err => setError(err?.response?.data?.message || 'Unable to load match info'))
  ...
  async function confirm(){
    ...
    try{
      const payload = {
        matchId,
        seats: selectedSeats.map(seat => ({ id: seat.id, label: seat.label, price: seat.price ?? match?.baseTicketPrice }))
      }
      const response = await bookingService.createBooking(payload)
      const bookingId = response?.data?.id || response?.data?.bookingId
      navigate('/payment', { state: { bookingId, matchId, selectedSeats, totalPrice } })
    }catch(err){
      ...
      <div className="card">
        ...
        <ul>
          {selectedSeats.map(seat => (
            <li key={seat.id}>{seat.label} — ${(seat.price ?? match?.baseTicketPrice ?? 0).toFixed(2)}</li>
          ))}
        </ul>
