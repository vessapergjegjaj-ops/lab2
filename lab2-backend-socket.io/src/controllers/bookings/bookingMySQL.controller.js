...
const bookingMySQLController = {
  ...
  async createBooking(req, res) {
    try {
      ...
      socketHub.emit('bookingCreated', booking);
      socketHub.emitDashboardUpdated({ reason: 'bookingCreated', booking });
  ...
  async updateBooking(req, res) {
    try {
      ...
      socketHub.emitDashboardUpdated({ reason: 'bookingUpdated', booking });
      res.json({ success: true, data: booking });
    } catch (error) {
      res.status(400).json({ success: false, error: error.message });
    }
